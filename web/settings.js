...
const VIDEO_SOURCE_CONFIG = 'VIDEOSOURCECONFIG';
...
const VideoSourceSelectContainer = document.getElementById('video_source_select_container');
...
function initConfig() {
  (async () => {
    ...
    if (config) {
      ...
      // Video
      const videoSourceConfig = config[VIDEO_SOURCE_CONFIG];
      initVideoSource(videoSourceConfig['video_source']);
      // Translation
      ...
      initSetTranslationLanguages({
        sourceLang: translationConfig['source_lang'],
        targetLang: translationConfig['target_lang'],
      });
      // Logs
      ...
      initSetImageResize({
        isResizeScreenshot: logConfig['resize_screenshot'],
        screenshotMaxWidth: logConfig['resize_screenshot_max_width'],
        screenshotMaxHeight: logConfig['resize_screenshot_max_height'],
      });
...
function initOCREngine(engine) {
  if (engine) {
    ...
    if (selectedOption) {
      ...
      const defaultOption = Array.from(engineOptions).find(
        (child) => child.innerText == 'Tesseract Default'
      );
...
function initVideoSource(video_source) {
  if (video_source) {
    videoSource = video_source;
    const videoSourceOptions = VideoSourceSelectContainer.querySelectorAll('li');
    const selectedOption = Array.from(videoSourceOptions).find(
      (child) => child.innerHTML === video_source
    );
    if (selectedOption) {
      selectedOption.setAttribute('data-selected', true);
    } else {
      // Fallback to default Application Window if option not found
      const defaultOption = Array.from(videoSourceOptions).find(
        (child) => child.innerText == 'Application Window'
      );
      defaultOption.setAttribute('data-selected', true);
    }
    getmdlSelect.init('#video_source_select_container');
  }
}
...
function updateVideoSource() {
  ...
  // if (videoSource.includes('Capture Card')) {
  ...
  //   if (VideoSource.includes('Tesseract')) {
  //     // Enable Tesseract Features
  //     textOrientationSwitch.disabled = false;
  //     textOrientationSwitch.parentNode.classList.remove('is-disabled');
  //   } else {
...
function updateVideoSourceAndPersist() {
  const VideoSource = updateVideoSource();
  eel.update_config(VIDEO_SOURCE_CONFIG, { video_source: VideoSource })();
  //   if (currentConfig[VIDEO_SOURCE_CONFIG]['video_source'] !== VideoSource) {
  //     eel.update_config(VIDEO_SOURCE_CONFIG, { video_source: VideoSource })();
  //   }
}
...
async function setCardModel() {
  ...
  const selectedOption = Array.from(cardModelOptions).find(
    (child) => child.value === defaultCardModel
  );
  if (selectedOption) {
    selectedOption.setAttribute('data-selected', true);
...
function changeCardModel() {
  if (ankiModelFieldMap[cardModelSelect.value]) {
    ...
    if (savedAnkiCardModels) {
      const existingModelIndex = savedAnkiCardModels.findIndex(
        (obj) => obj['model'] === selectedModel
      );
...
function initSetRemoveRepeatedMode(removeRepeatedMode) {
  if (removeRepeatedMode) {
    ...
    const removeRepeatOption = Array.from(removeRepeatOptions).find(
      (child) => child.innerText.toLowerCase() === removeRepeatedMode.toLowerCase()
    );
    if (removeRepeatOption) {
      removeRepeatOption.setAttribute('selected', true);
    } else {
      // Fallback to quick
      const defaultOption = Array.from(removeRepeatOptions).find(
        (child) => child.innerText.toLowerCase() == 'quick'
      );
...
function toggleRemoveDuplicateCharactersAndPersist() {
  ...
  eel.update_config(TEXTHOOKER_CONFIG, {
    remove_duplicates: isRemoveDuplicateCharacters ? 'true' : 'false',
  })();
}
